...
<html class="dark" lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>SmashCourt | Pembayaran {{ $payment->order_id }}</title>
        ...
        @php
            ...
            $statusPalette = match ($payment->status) {
                \App\Models\Payment::STATUS_SUCCESS => [
                    ...
                    'description' => 'Pembayaran sudah terverifikasi. Slot kamu aman dan booking tetap tercatat.',
                ],
                \App\Models\Payment::STATUS_FAILED => [
                    ...
                    'description' => 'Pembayaran sebelumnya belum berhasil. Kamu masih bisa coba lagi selama booking belum dibatalkan.',
                ],
                default => [
                    'label' => 'Menunggu Pembayaran',
                    'badge' => 'border-primary/30 bg-primary/10 text-primary',
                    'panel' => 'border-primary/30 bg-primary/10 text-white',
                    'description' => 'Sistem masih menunggu pembayaran selesai atau pembaruan status dari Midtrans.',
                ],
            };
        @endphp
        <main
            ...
            <div class="grid w-full gap-8 lg:grid-cols-[1.25fr_0.95fr]">
                <section class="overflow-hidden rounded-3xl border border-white/10 bg-surface shadow-2xl shadow-black/20">
                    <div class="border-b border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(195,244,0,0.22),_transparent_35%),linear-gradient(135deg,_rgba(179,197,255,0.18),_transparent_65%)] px-6 py-8 md:px-10">
                        <p class="text-sm font-semibold uppercase tracking-[0.25em] text-accent">Pembayaran Aman</p>
                        <h1 class="mt-3 font-display text-3xl font-extrabold uppercase tracking-tight text-white md:text-5xl">Selesaikan Pembayaran</h1>
                        <p class="mt-4 max-w-2xl text-sm leading-7 text-muted md:text-base">
                            Booking kamu sudah dibuat dan slot sudah ditahan sementara. Lanjutkan pembayaran lewat Midtrans untuk mengunci jadwal.
                        </p>
                    </div>
                    ...
                    <div class="space-y-8 px-6 py-8 md:px-10">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="rounded-2xl border border-white/10 bg-panel px-5 py-5">
                                <p class="text-xs font-semibold uppercase tracking-[0.22em] text-muted">Kode Booking</p>
                                <p class="mt-2 text-xl font-semibold text-white">{{ $booking->booking_code }}</p>
                            </div>
                            <div class="rounded-2xl border border-white/10 bg-panel px-5 py-5">
                                <p class="text-xs font-semibold uppercase tracking-[0.22em] text-muted">Status Pembayaran</p>
                        ...
                        <div id="payment-status-panel" class="rounded-3xl border px-5 py-5 {{ $statusPalette['panel'] }}">
                            <p class="text-xs font-semibold uppercase tracking-[0.22em]">Ringkasan Status</p>
                            ...
                            <div class="mt-5 grid gap-3 sm:grid-cols-2">
                                <div class="rounded-2xl border border-white/10 bg-black/10 px-4 py-4">
                                    <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-inherit/70">Batas Waktu</p>
                                    <p id="booking-expiry-countdown" class="mt-2 font-display text-2xl font-bold text-white">
                                        --
                                    </p>
                                </div>
                                <div class="rounded-2xl border border-white/10 bg-black/10 px-4 py-4">
                                    <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-inherit/70">Berakhir</p>
                        ...
                        <div class="rounded-3xl border border-white/10 bg-panel px-5 py-6">
                            <h2 class="font-display text-2xl font-bold uppercase tracking-tight text-white">Ringkasan Booking</h2>
                            ...
                            <div class="mt-6 space-y-4 text-sm text-muted">
                                ...
                                <div class="border-t border-white/10 pt-4">
                                    <div class="flex items-center justify-between gap-4">
                                        <span class="text-xs font-semibold uppercase tracking-[0.22em] text-muted">Total Bayar</span>
                        ...
                        <div class="rounded-3xl border border-white/10 bg-panelHigh px-5 py-6">
                            <h2 class="font-display text-xl font-bold uppercase tracking-tight text-white">Catatan Pembayaran</h2>
                            <ul class="mt-4 space-y-3 text-sm leading-6 text-muted">
                                <li>Pembayaran diproses di halaman resmi Midtrans.</li>
                                <li>Status booking akan diperbarui otomatis setelah callback diterima.</li>
                                <li>Jangan tutup tab sebelum proses pembayaran selesai.</li>
                            </ul>
                        </div>
                    </div>
                </section>
                ...
                <aside class="rounded-3xl border border-white/10 bg-surface p-6 shadow-2xl shadow-black/20 md:p-8">
                    <h2 class="font-display text-2xl font-bold uppercase tracking-tight text-white">Langkah Berikutnya</h2>
                    <p class="mt-3 text-sm leading-7 text-muted">
                        Kamu akan diarahkan ke halaman pembayaran Midtrans. Cek lagi order ID dan nominalnya sebelum bayar.
                    </p>
                    ...
                    <div id="payment-action-area">
                        @if ($paymentStatus === \App\Models\Payment::STATUS_SUCCESS)
                            <div class="mt-8 rounded-2xl border border-accent/30 bg-accent/10 px-4 py-5 text-sm leading-6 text-white">
                                Pembayaran sudah sukses. Kamu tidak perlu menekan tombol bayar lagi.
                            </div>
                            <a
                        ...
                        @elseif ($bookingStatus === \App\Models\Booking::STATUS_PENDING)
                            ...
                            <p class="mt-4 text-xs leading-6 text-muted">
                                Kalau sesi lama gagal, sistem akan membuat sesi pembayaran baru yang tetap terhubung ke booking yang sama.
                            </p>
                        @else
                            <div class="mt-8 rounded-2xl border border-danger/30 bg-danger/10 px-4 py-4 text-sm leading-6 text-danger">
                                Booking ini sudah tidak bisa diproses lagi dari halaman ini.
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p id="payment-live-status" class="mt-2 text-sm text-white">
                                    {{ $paymentStatus === \App\Models\Payment::STATUS_PENDING ? 'Memantau status pembayaran secara otomatis...' : 'Status sudah sinkron.' }}
                                </p>
                            </div>
                            <button
            ...
            if (paymentPage) {
                ...
                const statusMap = {
                    success: {
                        ...
                        panel: 'border-accent/30 bg-accent/10 text-white',
                        label: 'Pembayaran Dikonfirmasi',
                        description: 'Pembayaran berhasil diverifikasi. Slot kamu sudah aman dan booking sudah tercatat lunas.',
                        liveText: 'Pembayaran berhasil dan status halaman ini sudah sinkron.',
                    },
                    failed: {
                        badge: 'border-danger/30 bg-danger/10 text-danger',
                        panel: 'border-danger/30 bg-danger/10 text-danger',
                    ...
                    pending: {
                        ...
                        label: 'Menunggu Pembayaran',
                        description: 'Sistem masih menunggu pembayaran selesai atau pembaruan status dari Midtrans.',
                        liveText: 'Memantau status pembayaran secara otomatis...',
                    },
                };
